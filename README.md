...
- Markdown ファイル（`.md` / `.markdown`）のプレビュー
  - Markdown ファイルは初期表示が `markdown` モード
- プレビューモード切り替え（`v`）
  - Markdown: `markdown -> raw -> diff`（差分がある場合）
  - それ以外のテキスト: `raw <-> diff`（差分がある場合）
...
- Markdown プレビューは Git 管理外ディレクトリでも利用できます。
