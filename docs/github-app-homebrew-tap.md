...
このドキュメントは、`homebrew-sync` という GitHub App を使って、`pocoshelf` など複数リポジトリの release workflow から Homebrew tap を自動更新するための手順をまとめたものです。
...
- `rc-code-jp/pocoshelf` と `rc-code-jp/homebrew-tap` が存在していること
- GitHub 上で `rc-code-jp` の設定を変更できること
- `pocoshelf` を含む 1 つ以上の release workflow から `homebrew-tap` を更新したいこと
...
- `Homepage URL`: `https://github.com/rc-code-jp/pocoshelf`
...
通常はこれで十分です。`Formula/pocoshelf.rb` を更新するだけなら、`Workflows` 権限は不要です。
...
- `rc-code-jp/pocoshelf`
- `rc-code-jp/homebrew-tap`

`pocoshelf` 側で workflow を実行し、その token で `homebrew-tap` を更新するため、この 2 リポジトリに入れておくのが扱いやすい構成です。将来ほかのアプリでも使う場合は、その release workflow を持つ各リポジトリも同じ App のインストール対象に追加します。

## 6. 各アプリのリポジトリに Actions secrets を追加する

まず `rc-code-jp/pocoshelf` の次の画面を開きます。
...
その後に `Formula/pocoshelf.rb` の `version` と `sha256` を更新して commit / push します。

## 8. なぜ `GITHUB_TOKEN` ではなく GitHub App なのか

`GITHUB_TOKEN` は基本的に workflow が動いている同じリポジトリ向けです。今回のように `pocoshelf` や他のアプリから別リポジトリの `homebrew-tap` へ安全に書き込みたい場合は、GitHub 公式の案内どおり GitHub App を使うのが自然です。
...
GitHub App の設定が終わったら、`pocoshelf` 側で次を実装します。

- release workflow で GitHub App token を発行する
- `rc-code-jp/homebrew-tap` を checkout する
- `Formula/pocoshelf.rb` の `version` と `sha256` を更新する
